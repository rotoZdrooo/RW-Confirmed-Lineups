"""
woc_lineup_bot.py
Fetches World Cup lineup data from Rotowire and posts to Discord.
- Uses Nickname field from API for display names (falls back to Firstname Lastname)
- X (Predicted) lineups → DISCORD_WEBHOOK_WOC_PREDICTED channel
- C (Confirmed) lineups → DISCORD_WEBHOOK_WOC_CONFIRMED channel
- When a lineup upgrades X → C, posts to the confirmed channel
...
WEBHOOK_PREDICTED = os.environ.get("DISCORD_WEBHOOK_WOC_PREDICTED", "")
...
def build_message(game: dict, team: dict) -> str:
    ...
    emoji, label = STATUS_DISPLAY.get(team["status"], ("📋", team["status"]))
    lines = [
        f"⚽ **World Cup Lineup**",
        f"🏟️  **{team_names}**",
        f"🗓️  {kickoff}",
        "",
        f"{emoji} **{team['name']}** — *{label} XI*",
        "",
        "**Starting XI:**",
    ]
    ...
    return message
# ── Discord ───────────────────────────────────────────────────────────────────
def post_to_discord(webhook: str, message: str):
    try:
        r = requests.post(webhook, json={"content": message}, timeout=10)
        r.raise_for_status()
        print("[OK] Posted to Discord.")
        time.sleep(2)  # avoid Discord rate limit (5 req/sec)
    except Exception as e:
...
def main():
    if not API_KEY:
        ...
        return
    if not WEBHOOK_PREDICTED:
        print("[ERROR] DISCORD_WEBHOOK_WOC_PREDICTED is not set.")
        return
    if not WEBHOOK_CONFIRMED:
        print("[WARN] DISCORD_WEBHOOK_WOC_CONFIRMED is not set — confirmed lineups will be skipped.")
    ...
    for game in games:
        ...
        for team in game["teams"]:
            ...
            if status not in ("C", "X"):
                print(f"[SKIP] {team['name']} — unknown status '{status}'.")
                continue
            if not players:
            ...
            status_upgraded = (prev_status == "X" and status == "C")
            no_change       = (h == prev_hash and not status_upgraded)

            if no_change:
                print(f"[SKIP] {team['name']} — no change.")
                continue

            webhook = WEBHOOK_CONFIRMED if status == "C" else WEBHOOK_PREDICTED
            if not webhook:
                print(f"[SKIP] {team['name']} — webhook for '{status}' not configured yet.")
                continue

            reason = "X→C upgrade" if status_upgraded else ("new" if not prev_hash else "change")
            print(f"[POST] {team['name']} — {reason} → {'confirmed' if status == 'C' else 'predicted'} channel.")
            post_to_discord(webhook, build_message(game, team))
            state[state_key] = {"hash": h, "status": status}
            posted_count += 1

    save_state(state)
    print(f"Done. Posted/updated {posted_count} lineup(s).")
